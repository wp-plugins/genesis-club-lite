 ...
 */
define('GENESIS_CLUB_VERSION','1.2');
...
require_once($dir . 'display-widgets.php');
...
if (is_admin()) {
    ...
    add_action ('init', 'genesis_club_admin_init',0);
}
...
function genesis_club_init() {
	if ( basename( TEMPLATEPATH ) == 'genesis' ) { //can only initialize plugin if Genesis
		...
		GenesisClubDisplayWidgets::init();		
		GenesisClubProfile::init();	
		GenesisClubLikebox::init();	
	}
}

function genesis_club_admin_init() {
	if ( basename( TEMPLATEPATH ) == 'genesis' ) { //can only initialize plugin admin if Genesis
		...
		add_action('admin_init',array('GenesisClubAdmin','deactivate')); 
	}
}
?>